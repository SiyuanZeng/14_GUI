...
public class SolrBaseDAO {
	...
	public HttpSolrServer getSolrConnection() throws Exception {
		...
		try {
			...
			if (solrServer == null) {
					...
					solrServer = new HttpSolrServer("http://localhost:8983/solr/jcg");
	...
	public HttpSolrServer getSolrConnectionfore() throws Exception {
		HttpSolrServer solrServer = null;
		try {
			// configure a server object with actual solr values.
			if (solrServer == null) {

				solrServer = new HttpSolrServer("http://localhost:8983/solr/coolection");
				solrServer.setParser(new XMLResponseParser());
				solrServer.setSoTimeout(5000);
				solrServer.setConnectionTimeout(5000);
